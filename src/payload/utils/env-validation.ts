...
export function validateEnvironment(): ValidationResult {
  ...
  ENV_VARIABLES.forEach(config => {
    ...
    if (config.required && !value) {
      ...
      result.errors.push(`Missing required environment variable: ${config.name} - ${config.description}`)
      result.valid = false
      return
    }
...
function isInsecureValue(name: string, value: string): boolean {
  ...
  const isSecretVar = name.toLowerCase().includes('secret') || 
...
function checkForUnexpectedSecrets(result: ValidationResult): void {
  ...
  const commonVars = new Set([
    'NODE_ENV', 'PORT', 'PWD', 'HOME', 'PATH', 'USER', 'SHELL',
    'NEXT_RUNTIME', 'NEXT_BUILD', 'VERCEL', 'VERCEL_URL', 'VERCEL_ENV',
    'CI', 'GITHUB_ACTIONS', 'RUNNER_OS', 'npm_package_name', 'npm_config_cache'
  ])
  ...
  Object.keys(process.env).forEach(key => {
    ...
    if (lowerKey.includes('secret') || 
      ...
      result.warnings.push(`Unexpected environment variable detected: ${key} - verify it's documented and secure`)
    }
  })
}
